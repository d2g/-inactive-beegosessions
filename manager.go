...
func NewManager(provideName, cookieName string, maxlifetime int64, savePath string, options ...interface{}) (*Manager, error) {
	provider, ok := providers.Provides[provideName]
	if !ok {
...
//get Session
func (manager *Manager) SessionStart(w http.ResponseWriter, r *http.Request) (session providers.SessionStore) {
...
func (manager *Manager) GetProvider(sid string) (sessions providers.SessionStore, err error) {
...
func (manager *Manager) SessionRegenerateId(w http.ResponseWriter, r *http.Request) (session providers.SessionStore) {
	sid := manager.sessionId(r)
	cookie, err := r.Cookie(manager.cookieName)
	if err != nil && cookie.Value == "" {
		//delete old cookie
